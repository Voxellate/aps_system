...
<h1>Coach Search</h1>
<?php
$sql = dbquery("SELECT * FROM coaches ORDER BY lastName");
if (mysqli_num_rows($sql) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($sql)) {
        echo "<tr><td>" . $row['firstName'] . "</td><td>" . $row['lastName'] . "</td><td>" . $row['email'] . "</td><td>" . $row['sportName'] . "</td><td>" . $row['teamName'] . "</td></tr>";
    }
    echo "</table>";
} else {
